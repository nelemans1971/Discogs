...
namespace CDR.Logging
{
    ...
    {
        static public CompositeLogger Logger;

        static private string filename = @".\\log.log";
        static private FileLogger fileLogger = null;
        static private MemoryLogger memoryLogger = null;
        ...
        static public bool MemoryLoggerContainsErrors
        {
            get
            {
                LogEntry[] logEntries = memoryLogger.GetLog();
                foreach (LogEntry log in logEntries)
                {
                    if (memoryLogger.Formatter.AsString(log).ToUpper().Contains("<ERROR>"))
                    {
                        return true;
                    }
                } //foreach
